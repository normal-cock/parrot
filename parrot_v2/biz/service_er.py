...
from parrot_v2.biz import service_v2
...
def add_er_lookup_record():
    ...
    meaning = rlinput("meaning:", meaning_obj.meaning if meaning_obj else "")
    use_case = rlinput("use case:", meaning_obj.use_case if meaning_obj else "")
    ...
    if word == None:
        ...
        update_meaning_fts(session, meaning_obj.id, old_meaning_usecase, meaning_obj)

    added_count = (
        session.query(ERLookupRecord)
        .filter(ERLookupRecord.created_time >= datetime.date.today())
        .count()
    )
    session.commit()
...
def _gen_er_lookup_records(session, begin_time, end_time) -> List[ERLookupRecord]:
    ...
    if len(lookup_record_id_list) == 0:
        lookup_records: List[ERLookupRecord] = (
            session.query(ERLookupRecord)
            .filter(
    ...
    er_lookup_records = (
        session.query(ERLookupRecord)
        .filter(ERLookupRecord.id.in_(lookup_record_id_list))
        .all()
    )
...
def fetch_next_er_lookup_record():
    ...
    er_lookup_records = _gen_er_lookup_records(session, begin_time, end_time)
    total_len = len(er_lookup_records)
    logger.info(f"lookup records since {begin_time} to {end_time}. Total {total_len}")
...
def begin_er_lookup_review():
    ...
    for index, lookup_record in enumerate(er_lookup_records):
        ...
        other_meanings: List[Meaning] = (
            meaning.word.get_other_meaning_without_review_plan(lookup_meaning_id_set)
        )
        other_meanings = [m for m in other_meanings if m.id != meaning.id]
