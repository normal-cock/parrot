...
SESSION_CACHELIB = FileSystemCache(threshold=500, cache_dir=f"{DATA_DIR}/sessions")
...
def item_play_page(passport, item_id):
    ...
    if not (
        "expiration_time" in media_url_dict
        and time.time() < media_url_dict["expiration_time"]
    ):
...
def query_word(passport, q):
    if passport.upper() != PW.upper():
        return make_response("", 404)
    query = q.strip()
    sentence = request.args.get("sentence", query)
    result_list = biz_v2.query_word(query, sentence)
    resp_list = []
    for result in result_list:
        resp_list.append(
            {
...
def parse_sentence(passport):
    ...
    for raw_word, qr_list in result_dict["unknown_words"].items():
        ...
        if len(qr_list) > 0:
            word = qr_list[0]["word"]
        resp_dict["unknown_words"].append(qr_result_gen(raw_word, word, qr_list))
    return resp_dict
...
def heartbeat(passport, item_id):
    ...
    if (
        new_hb_data == None
        or "version" not in new_hb_data
        or "cue_index" not in new_hb_data
        or "adjust_st" not in new_hb_data
        or "adjust_et" not in new_hb_data
    ):
...
def meaning_speech(passport, meaning_id):
    ...
    if len(meaning_speech_path) == 0:
        return make_response("", 404)
    return send_file(path_or_file=meaning_speech_path, mimetype="audio/mpeg")
