...
未来优化思路:
* (20251018)接入CDN优化成本。成本差不太多，CDN还需要配置域名之类的比较麻烦
    * CDN的流量费用0.24元/GB，OSS的CDN回源出流量费用0.15元/GB
    * OSS的出流量费用0.5元/GB
* [P0]提高碎片化时间的利用率
    * [P0]支持声音背单词，背单词就不需要电脑了。录入单词时，可以将句子的文件名和时间段存下来，第一版先只读句子就可以——Done，使用的思路2.3
...
* [P0]仅下一句时自动跳转，播放时不自动跳转，同时使用悬浮按钮来手动跳转——Done
* [P0]review页面支持prev和next——Done
* [P0]review页面自动播放——Done
* [P1]review页面通过下划线标识已入库单词，并支持点击显示详情
    * 后端接口直接生成好句子的html，能够点击查询并弹出查询结果
* [P1]提高背单词效率
